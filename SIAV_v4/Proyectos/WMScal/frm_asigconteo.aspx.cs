...
{
    public partial class frm_asigconteo : System.Web.UI.Page
    {
        ...
        public void VincularDdlUsuario()
        {
            ...
            dsp = an_wmscal.GetPicking(2, Request.Cookies["basesiav"].Value);
        ...
        public void GridProdFam()
        {
            try
            {
                if (op == 1)
                {
                    gvProdFam.DataSource = an_wmscal.GetCCproductofamilia(op, txtDato.Text.Trim());
                    gvProdFam.DataBind();
                }
                if (op == 2)
                {
        ...
        public void GridAsignados()
        {
            try
            {
                gvUsuAsig.DataSource = an_wmscal.GetUsuAsig("",0);
                gvUsuAsig.DataBind();
            }
            catch (Exception ex)
            {
        ...
        public void GridDetalle()
        {
            try
            {
                gvDetalle.DataSource = an_wmscal.GetUsuAsig(idMConteoCiclico, 3);
                gvDetalle.DataBind();
            }
            catch (Exception ex)
            {
                    ...
                    foreach (GridViewRow row in gvProdFam.Rows)
                    {
                        if (row.RowType == DataControlRowType.DataRow)
                        {
                            ...
                            if (chkRow.Checked)
                            {
                                ...
                                salida = an_wmscal.InsConteoCiclico(1,tipo,"",0,codigo,descripcion,"","",empresa,0,usuario,usuarioasigna);
                            }
                            else
                            {
        ...
        protected void btnContar_Click(object sender, EventArgs e)
        {
            try
            {
                lblError.Text = "";
                string empresa = Request.Cookies["basesiav"].Value;
                string salida = an_wmscal.InsConteoCiclico(8, tipo, "", 0, "", "", txtObservacion.Text.Trim(), "", empresa, 0, "", "");
