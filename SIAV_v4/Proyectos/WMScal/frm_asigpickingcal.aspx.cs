...
{
    public partial class frm_asigpickingcal : System.Web.UI.Page
    {
        ...
        protected void gvPedidosPicking_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                string iavec = (e.Row.Cells[11].Text);
                string pi = (e.Row.Cells[12].Text);
                foreach (TableCell cell in e.Row.Cells)
                {
                    if (tipodoc != "PV")
                    {
        ...
        protected void gvDetPed_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
        ...
        protected void lnkpedido_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //Llenar el grid
                gvDetPed.DataSource = an_wms.GetDetallePedGrid(pedido);
                gvDetPed.DataBind();
                //Abrir el modal
                System.Text.StringBuilder sb = new System.Text.StringBuilder();
                sb.Append(@"<script type='text/javascript'>");
