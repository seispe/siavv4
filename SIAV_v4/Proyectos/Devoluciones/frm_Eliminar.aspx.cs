...
{
    public partial class frm_Eliminar : System.Web.UI.Page
    {
        ...
        protected void gvEliminar_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                {
                    string code = gvEliminar.DataKeys[index].Value.ToString();
                    string estado = (gvEliminar.Rows[index].FindControl("lblestado") as Label).Text;
                    if (estado=="ELIMINADA")
                    {
                        lblError.Text = an_alertas.Mensaje("ERROR!  ", "La Devolucion ya se encuentra Eliminada.", "rojo");
                    }
                    else
                    {
        ...
        {
            if (txtidDevolucion.Text.Length > 0)
            {
                VincularGrid(Convert.ToInt32(txtidDevolucion.Text));
            }
            else
            {
                lblError.Text = an_alertas.Mensaje("ERROR!  ", "El # de devolucion es obligatorio.", "rojo");
            }
        }
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            if (txtObservacion.Text.Trim().Length > 0)
            {
                try
                {
                    ...
                    VincularGrid(Convert.ToInt32(HfDeleteID.Value));
                }
                catch (Exception ex)
                {
        ...
        public void VincularGrid(int iddevolucion)
        {
            gvEliminar.DataSource = an_devolucion.LlenarGrid(Request.Cookies["basesiav"].Value, iddevolucion).DataSource;
            gvEliminar.DataBind();
        }
        #endregion
    }
