...
{
    public partial class rpt_PackingListDevoluciones : System.Web.UI.Page
    {
        ...
        public void GridNE()
        {
            try
            {
                if (txtPedido.Text.Length > 0)
                {
                    gvNE.DataSource = an_wms.GetNE(txtPedido.Text.Trim(), 4).DataSource;
                    gvNE.DataBind();
                }
                else
                {
                    //ninguna accion
                    //gvNE.DataSource = an_wms.GetNE("", 4).DataSource;
                    //gvNE.DataBind();
                }
