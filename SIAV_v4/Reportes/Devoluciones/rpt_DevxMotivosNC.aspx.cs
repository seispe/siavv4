...
{
    public partial class rpt_DevxMotivosNC : System.Web.UI.Page
    {
        ...
        public void VincularGrid(string empresa, string fechaDesde, string fechaHasta, string cliente, string iddevolucion)
        {
            try
            {
                gvDev.DataSource = an_devolucion.rpt_pormotivoNC(empresa, fechaDesde, fechaHasta, cliente, iddevolucion).DataSource;
                gvDev.DataBind();
            }
            catch (Exception EX)
            {
        ...
        protected void ExportToExcel(object sender, EventArgs e)
        {
            ...
            if (Request.Cookies["basesiav"].Value == "GPIAV")
            {
                ...
                GridView1.DataSource = an_devolucion.rpt_pormotivoNC(Request.Cookies["basesiav"].Value, fechadesde, fechahasta, txtCliente.Text.Trim(), txtNumDevol.Text.Trim()).DataSource;
                GridView1.DataBind();
            }
            else
            {
                GridView1.DataSource = an_devolucion.rpt_pormotivoNC(Request.Cookies["basesiav"].Value, txtFechaDesde.Text.Trim(), txtFechaHasta.Text.Trim(), txtCliente.Text.Trim(), txtNumDevol.Text.Trim()).DataSource;
                GridView1.DataBind();
            }
            Response.Clear();
