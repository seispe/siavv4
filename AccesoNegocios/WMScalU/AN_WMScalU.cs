...
namespace AccesoNegocios.WMScalU
{
    public class AN_WMScalU
    {
        ...
        public GridView GetBultosxDia(string desde, string hasta)
        {
            DataSet dsp = new DataSet();
            GridView gv = new GridView();

            dsp = ad_wms.GetBultosxDia(desde, hasta);
        ...
        public string LiberaEtiquetas(string pedido)
        {
            try
            {
                return ad_wms.LiberaEtiquetas(pedido);
            }
            catch (Exception ex)
            {
                throw ex;
